...
function serviceSpecsToMongoose (feathersSpec, feathersExtension, depth = 1) {
  ...
  const uniqueItemProperties = _merge([], feathersSpec.uniqueItemProperties) || [];
  ...
  Object.keys(properties).forEach(name => {
    ...
    if (mongooseProperty.description) {
      mongooseProperty.description;
    }
...
function toMongooseType (srcType, defaultType) {
  return typeEquivalence[srcType] ||
  defaultType ||
  String;
}
